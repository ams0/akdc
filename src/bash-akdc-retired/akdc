...
function akdc_usage() {
    ...
    echo "    delete  -- delete an Azure Resource Group and associated Azure DNS record"
...
function akdc_exec() {
    # shellcheck disable=2002,SC2013
...
case $1 in
    check)
        case $2 in
            ...
            flux)
                ...
                ;;
            ngsa)
                akdc_exec_task 'if [[ $(kubectl get pods -A) == *"ngsa"* ]]; then echo "$(hostname) found"; else echo "$(hostname) not found"; fi'
                ;;
            retries)
            ...
            setup)
                ...
                ;;
            ssl)
                akdc_exec_task 'echo "$(hostname) $(curl -s https://$(hostname).cseretail.com/tinybench/17)"'
                ;;
            *)
                ...
                echo "    ssl      -- curl tinybench via https on each server"
                ;;
        esac
        ;;
    exec)
    ...
    groups)
        az group list -o table | sort | grep -e central- -e east- -e west- -e corp-
        ;;
    ...
    delete)
        ...
        serverName=$2
        response=$(az group list --query "[?tags.server == '$serverName']" -o json)
        resourceGroupName=$(echo "$response" | jq -r '.[].name')
        zone=$(echo "$response" | jq -r '.[].tags.zone')

        echo "Deleting Resource Group: $resourceGroupName"
        az group delete -n "$resourceGroupName" --yes --no-wait

        if [ "$zone" != "null" ]; then
            echo "Deleting Record: $resourceGroupName in Zone:$zone"
            az network dns record-set a delete -g tld -z "$zone" --yes -n "$resourceGroupName"
        fi
        ;;
    *)
        akdc_usage
