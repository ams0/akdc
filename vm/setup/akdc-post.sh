...
if [[ "$(hostname)" == *"monitor"* ]]
then
    docker run -d --restart always --name webv-heartbeat -p 30088:8080 ghcr.io/cse-labs/webv-red:beta -s https://central-tx-atx-510.cseretail.com https://east-ga-atl-510.cseretail.com https://west-wa-sea-510.cseretail.com -f heartbeat-benchmark.json --run-loop --prometheus --sleep 5000
else
